    ...
    script:
      """
        ...
        python ${baseDir}/scripts/sqanti3_qc_chunk_run.py \
              ...
              -d qc_chunk_\${CHUNK_IDX}/ --include_ORF --output ${prefix} --report skip
      """
}
...
process SQANTI3_QC_COMBINE {
    ...
    publishDir (params.sqanti3_output_dir ?: "${params.results_output}results/transcript_info/sqanti3/"), mode: 'copy', overwrite: true
    ...
    output:
      ...
      path("sqanti3_qc/${prefix}_classification.txt"),       emit: classification
      path("sqanti3_qc/${prefix}_corrected.gtf"),            emit: corrected_gtf
      path("sqanti3_qc/${prefix}_corrected.fasta"),          emit: corrected_fasta
      path("sqanti3_qc/${prefix}_corrected.faa"),            emit: corrected_faa
    ...
    script:
      """
        python ${baseDir}/scripts/sqanti3_qc_combine.py \
              ...
              --chunks ${num_chunks} -t ${task.cpus} \
              -d sqanti3_qc/ --include_ORF --output ${prefix} --report pdf \
...
process SQANTI3_FILTER {
  ...
  output:
    ...
    path("sqanti3_filter/${prefix}_pass_isoforms.txt"),               emit: pass_isoforms
    path("sqanti3_filter/${prefix}_corrected.filtered.gtf"),          emit: filtered_gtf
    path("sqanti3_filter/${prefix}.filtered.gtf"),                    emit: filtered_original_gtf
  ...
  python ${baseDir}/scripts/gtf_subset.py -g ${corrected_gtf_f} -i sqanti3_filter/${prefix}_pass_isoforms.txt -o sqanti3_filter/${prefix}_corrected.filtered.gtf
  python ${baseDir}/scripts/gtf_subset.py -g ${input_gtf_f} -i sqanti3_filter/${prefix}_pass_isoforms.txt -o sqanti3_filter/${prefix}.filtered.gtf
  python ${baseDir}/scripts/td2_filter.py -d ${td2_dir} -i sqanti3_filter/${prefix}_pass_isoforms.txt -o sqanti3_filter/TD2
  """
